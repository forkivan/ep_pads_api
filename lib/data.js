...
// Returns: [{ id, lastEdited, lastEditedTs, creator }]
//   lastEdited   — ISO date (YYYY-MM-DD) for display
//   lastEditedTs — full epoch-ms timestamp for precise (to-the-minute) sorting
async function collectPads() {
  ...
  for (const id of padIDs) {
    ...
    try {
      ...
      if (ts) {
        lastEditedTs = Number(ts);
        lastEdited = new Date(ts).toISOString().slice(0, 10);
      }
    ...
    pads.push({ id, lastEdited, lastEditedTs, creator });
  }
